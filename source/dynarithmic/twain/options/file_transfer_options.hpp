...
namespace dynarithmic
{
    namespace twain
    {
        ...
        class file_transfer_options
        {
            ...
            bool m_bMultiPage;
            filename_increment_rules m_file_increment_rules;
            multipage_save_options m_multipage_save_options;

            public:
                file_transfer_options() : m_file_type(filetype_value::bmp),
                                    m_file_pattern("temp.bmp"), m_bMultiPage(false),
                                    m_transferFlags(file_transfer_flags::use_name)
                {}

                file_transfer_options& set_file_type(filetype_value::value_type ft)
                { m_file_type = ft; return *this; }

                file_transfer_options& set_filename_pattern(const std::string& name)
                { m_file_pattern = name; return *this; }

                filename_increment_rules& get_filename_increment_rules() { return m_file_increment_rules; }
                multipage_save_options& get_multipage_save_options() { return m_multipage_save_options; }

                filetype_value::value_type get_file_type() const { return m_file_type; }
                std::string get_filename_pattern() const { return m_file_pattern; }

                bool can_multi_page() const { return m_bMultiPage && file_type_info::get_multipage_type(m_file_type); }
                long get_multi_page_type() const { return file_type_info::get_multipage_type(m_file_type); }

                file_transfer_options& set_file_transfer_flags(file_transfer_flags ftf)
                { m_transferFlags = ftf; return *this; }

                file_transfer_flags get_file_transfer_flags() const { return m_transferFlags; }
