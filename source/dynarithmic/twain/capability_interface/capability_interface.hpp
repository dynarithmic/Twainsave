...
namespace twain {
    ...
    static std::vector<const char *> to_string(Iter it1, Iter it2, const typename std::map<typename const std::iterator_traits<Iter>::value_type, const char*>& sMap)
    {
    ...
    static std::vector<const char *> to_string(const Container& c, const typename std::map<const T, const char*>& sMap)
    {
        return to_string(std::begin(c), std::end(c), sMap);
    }

    template <typename T>
    static const char * to_string(const T& value, const typename std::map<const T, const char*>& sMap)
    {
    ...
    * pixel or bit depth, the dots-per-inch, x or y resolution, author name, etc.
    ...
    class capability_interface
    {
        ...
        capability_interface& capability_interface::operator= (capability_interface&& rhs) noexcept
        {
            if (&rhs != this)
            {
        ...
        capability_interface::capability_interface() : m_Source(nullptr), m_return_type{}
        {}
        ...
        class setcap_operation_info
        {
            public:
                ...
                setcap_operation_info& set_setter_type(set_operation_type::value_type st)
    ...
    private:
            ...
            const getcap_operation_info& gcType = getcap_operation_info()) const
        {
            ...
            twain_array ta;
            bool retVal = API_INSTANCE DTWAIN_GetCapValues(m_Source, capvalue,
                static_cast<LONG>(gcType.get_operation()), ta.get_array_ptr()) != 0;
            if (!retVal)
    ...
    public:
        ...
        {
            return setcap_operation_info().set_setter_type(set_operation_type::SET);
        }
            
        static setcap_operation_info reset()
        {
            return setcap_operation_info().set_setter_type(set_operation_type::RESET);
        }

        static setcap_operation_info reset_all()
        {
            return setcap_operation_info().set_setter_type(set_operation_type::RESET_ALL);
        }
            
        static setcap_operation_info set_constraint()
        {
            return setcap_operation_info().set_setter_type(set_operation_type::SET_CONSTRAINT);
        }
                                        ...
                                        const setcap_operation_info& scType = setcap_operation_info()) const
        {
            ...
            auto retval = API_INSTANCE DTWAIN_SetCapValues(theSource, capvalue, static_cast<LONG>(scType.get_operation()),
                                                ta.get_array());
            ...
            if (is_cache)
            {
                ...
                if (iter != m_cap_cache.end())
                {
                    ...
                    else
                    {
                        ...
                        twain_range<dtwain_underlying_type<CapType>::value_type> tr(vc.begin(), vc.end());
                        bool found = tr.value_exists(static_cast<dtwain_underlying_type<CapType>::value_type>(capvalue));
                        if ( found )
                            return {found, 1};
                        return {found, 0};
                    }
        ...
        bool is_capvalue_supported(const typename Cap::value_type& capValue, CAP_SUPPORTEDCAPS_::value_type capToTest) const
        {
            ...
            if ( !ret.second )
            {
                auto vect = get_custom<std::vector<typename Cap::value_type>>(capToTest); 
                auto cType = get_cap_container_type(capToTest, get());
                if ( cType != twain_container_type::CONTAINER_RANGE)
                {
                     bool found = std::find(vect.begin(), vect.end(), 
                            static_cast<dtwain_underlying_type<typename Cap::value_type>::value_type>(capValue)) != vect.end();
                     if ( found )
                        return true;
                     return false;
                }
                else
                {
                    ...
                    bool found = tr.value_exists(static_cast<dtwain_underlying_type<Cap::value_type>::value_type>(capValue));
                    if ( found )
                        return true;
                    return false;
                }
        ...
        bool is_capvalue_supported(const std::string& capValue, CAP_SUPPORTEDCAPS_::value_type capToTest) const
        {
            ...
            if ( !ret.second )
            {
                auto vect = get_custom<std::vector<std::string>>(capToTest); 
